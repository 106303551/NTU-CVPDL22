...
def build_city2foggy_cocostyle_source(image_set, args):
    ...
    val_root = Path(args.valid_dir)
    PATHS = {
        "train": (train_root / 'org' / "train", train_root / 'org' /  'train.coco.json'),
        "val": (val_root / 'fog' / "val", val_root / 'fog' / 'val.coco.json'),
    }
    ...
    dataset = CocoDetection(img_folder, ann_file, is_foggy=False, transforms=img_transform, return_masks=args.masks,
...
def build_city2foggy_cocostyle(image_set, args):
    ...
    val_root = Path(args.valid_dir)
    PATHS = {
        "train_src":  (train_root / 'org' / "train", train_root / 'org' /  'train.coco.json'),
        "train_tgt":  (train_root / 'fog' / "train",train_root / 'org' /  'train.coco.json'),
        "val": (val_root / 'fog' / "val", val_root / 'fog' / 'val.coco.json'),
        "test":(root / 'fog' / "public_test",root/'public_test_data.json')
    }
