...
@torch.no_grad()
def test(model,postprocessors,data_loader,device,result_dir):
    model.eval()
    imgs=data_loader.batch_sampler.sampler.data_source.coco.imgs
    save_dict={}
    for samples,targets in data_loader:
        ...
        for idx,result in enumerate(results):
            ...
            for k in result:
                new_result[k]  = result[k].detach().cpu().numpy().tolist()
            save_dict[file_name] = new_result 
    dir = os.path.dirname(result_dir)
...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir,result_dir):
    model.eval()
    criterion.eval()
    imgs=data_loader.batch_sampler.sampler.data_source.coco.imgs
    save_dict={}
